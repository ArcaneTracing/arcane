...
import { useOrganisationAuditLogs } from '@/hooks/organisations/use-organisation-config-query'
...
export function AuditTab() {
  ...
  const { data: auditLogsResponse, isLoading: isLoadingAuditLogs } = useOrganisationAuditLogs({
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-4 mb-4">
        <div className="flex-1">
          ...
          <Input
            id="audit-action"
            placeholder="e.g., organisation.user.added, organisation.*"
            value={actionFilter}
            onChange={(e) => setActionFilter(e.target.value)}
          />
          <p className="text-xs text-muted-foreground mt-1">
            Filter by action pattern (e.g., organisation.* for all org actions)
          </p>
        </div>
      </div>
