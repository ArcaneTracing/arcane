...
import { useImportConversationConfigurations } from "@/hooks/conversation/use-conversation-query";
...
export interface ImportConversationConfigDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  trigger?: React.ReactNode;
}

export function ImportConversationConfigDialog({ open, onOpenChange, trigger }: Readonly<ImportConversationConfigDialogProps>) {
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const [isDragging, setIsDragging] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);
  const fileInputId = useId();

  const importMutation = useImportConversationConfigurations();
  ...
  const mutationAction = useMutationAction({
    ...
    onSuccess: () => {
      ...
      onOpenChange(false);
      showSuccessToast('Conversation configuration imported successfully');
    },
    showErrorToast: true
  });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.error('Error importing conversation configurations', err);
    }
  };
          ...
          <form onSubmit={handleSubmit}>
            <div className="px-6 pt-8 pb-6 flex items-center gap-4">
              <div className="inline-flex shrink-0">
                <img src="/images/icons/new_project_icon.png" alt="Import Conversation Configurations" className="h-24 w-24" />
              </div>
              <div className="text-left">
                <DialogTitle className="text-xl font-semibold mb-2 dark:text-white">
                  Import Conversation Configurations
                </DialogTitle>
                <DialogDescription className="text-sm text-gray-500 dark:text-gray-400">
                  {conversationTooltips.import.dialogHeader}
                </DialogDescription>
              </div>
            </div>
