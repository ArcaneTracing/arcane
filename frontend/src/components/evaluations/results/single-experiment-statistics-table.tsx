...
interface SingleExperimentStatisticsTableProps {
...
export function SingleExperimentStatisticsTable({
  statistics,
  scores,
  evaluation,
  experimentId
}: Readonly<SingleExperimentStatisticsTableProps>) {

  const experimentStatistics = statistics.filter((stat) => stat.experimentId === experimentId);


  const scoreMap = new Map(scores.map((score) => [score.id, score]));


  const scoreIds = Array.from(new Set(experimentStatistics.map((s) => s.scoreId)));


  const statisticsMap = new Map<string, EvaluationStatisticsResponse>();
  experimentStatistics.forEach((stat) => {
  ...
  const renderNumericRow = (scoreId: string, stat: EvaluationStatisticsResponse) => {
  ...
  const renderNominalRow = (scoreId: string, stat: EvaluationStatisticsResponse) => {
    const scoreName = getScoreName(scoreId);
    const truncatedScoreName = truncateText(scoreName, 20);
  ...
  const renderOrdinalRow = (scoreId: string, stat: EvaluationStatisticsResponse) => {
    const scoreName = getScoreName(scoreId);
    const truncatedScoreName = truncateText(scoreName, 20);
