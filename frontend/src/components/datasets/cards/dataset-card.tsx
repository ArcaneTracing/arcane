...
import { Database, ArrowRight, Pencil, Trash2 } from "lucide-react"
import { PermissionGuard } from "@/components/PermissionGuard"
import { PERMISSIONS } from "@/lib/permissions"
import { useOrganisationId } from "@/hooks/useOrganisation"

interface DatasetCardProps {
  dataset: DatasetListItemResponse
  onView: (datasetId: string) => void
  onEdit: (dataset: DatasetListItemResponse) => void
  onDelete: (datasetId: string) => void
  projectId?: string
}

export function DatasetCard({ dataset, onView, onEdit, onDelete, projectId }: Readonly<DatasetCardProps>) {
    ...
    <Card className="border-gray-100 dark:border-[#2A2A2A] bg-white dark:bg-[#0D0D0D] hover:shadow-md transition-shadow">
      ...
      <CardContent className="pt-0">
        <div className="flex items-center justify-end gap-2">
          ...
          <PermissionGuard
            permission={PERMISSIONS.DATASET.UPDATE}
          ...
          <PermissionGuard
            permission={PERMISSIONS.DATASET.DELETE}
            organisationId={organisationId}
            projectId={projectId}
          >
            <Button
