...
import { useAnnotationQueuesByTypeQuery, useAnnotationQueueQuery, useAddConversation } from "@/hooks/annotation-queues/use-annotation-queues-query";
...
interface AnnotateConversationDrawerProps {
  projectId: string;
...
export function AnnotateConversationDrawer({
  projectId,
  conversationId,
  ...
  onClose,
  onComplete
}: Readonly<AnnotateConversationDrawerProps>) {
  ...
  const { data: conversationQueues = [], isLoading: isQueuesLoading } = useAnnotationQueuesByTypeQuery(
    projectId,
    isOpen ? AnnotationQueueType.CONVERSATIONS : undefined
  );
  ...
  useEffect(() => {
    if (queue && conversationId) {
      const existing = queue.annotations?.find((ann: AnnotationResponse) => ann.conversationId === conversationId || ann.otelConversationId === conversationId);
  ...
  }, [existingAnnotation, selectedQueueId]);
  ...
  useEffect(() => {
    if (!isOpen) {
      ...
      if (addConversationMutation.isPending) {
        addConversationMutation.reset();
      }
    }
  ...
  const handleSubmit = async () => {
    if (!template || !selectedQueueId) {
      actionError.handleError(new Error('Template and queue selection are required'));
      return;
    }
    ...
    try {
      ...
      if (existingAnnotation?.id) {
        ...
        const result = await addConversationMutation.mutateAsync({
          queueId: selectedQueueId,
          decodeTraceIds: true,
          data: {
            conversationConfigId,
            datasourceId,
            otelConversationId: conversationId,
        ...
        if (!result?.id) {
          throw new Error('Failed to add conversation to queue: Invalid response');
        }

        await createAnnotation({
          queueConversationId: result.id,
          startDate,
          endDate,
          answers: annotationAnswers
        });
  ...
  const isSubmitting = isCreateLoading || isUpdateLoading || addConversationMutation.isPending;
  ...
  return (
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent side="right" className="w-full sm:max-w-2xl overflow-y-auto">
        <SheetHeader>
          <SheetTitle>{existingAnnotation ? "View/Edit Annotation" : "Annotate Conversation"}</SheetTitle>
          <SheetDescription>
            {existingAnnotation ?
            `Review and edit the annotation for conversation ${conversationId}` :
            `Select an annotation queue and answer the questions to annotate conversation ${conversationId}`
            }
          </SheetDescription>
        </SheetHeader>
            ...
            <Select value={selectedQueueId} onValueChange={setSelectedQueueId}>
              ...
              <SelectContent>
                {(() => {
                  ...
                  if (conversationQueues.length === 0) return <div className="p-2 text-sm text-muted-foreground">No conversation annotation queues found</div>;
                  return conversationQueues.map((queue) =>
