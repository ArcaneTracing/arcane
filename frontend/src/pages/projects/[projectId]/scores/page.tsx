...
import { ScoreDialog } from "@/components/scores/dialogs/score-dialog"
...
const SCORE_SORT_OPTIONS: SortOption[] = [
  ...
  { value: 'scoringType', label: 'Scoring Type' },
  { value: 'createdAt', label: 'Created Date' },
]

export function ScoresPageContent() {
  const { projectId, organisationId } = useParams({ from: "/appLayout/organisations/$organisationId/projects/$projectId/scores", strict: false })
  ...
  return (
    <div className="p-10">
      <div className="flex justify-between items-center mb-8">
        <div> 
          <h1 className="text-2xl font-semibold tracking-tight mb-1.5">Scores Management</h1>
          <p className="text-sm text-muted-foreground/60">Create and manage scoring criteria for evaluating AI model outputs.</p>
        </div>  
      </div>
      <div className="flex items-center justify-between gap-4 mb-4">
        ...
        <div className="flex gap-2">
          <PermissionGuard
            permission={PERMISSIONS.SCORE.CREATE}
...
export default function ScoresPage() {
  return (
    <PagePermissionGuard permission={PERMISSIONS.SCORE.READ} fallback={<ForbiddenPage />}>
      <ScoresPageContent />
    </PagePermissionGuard>
  )
}
