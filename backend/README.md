...
This package is part of the **[Arcane monorepo](https://github.com/ArcaneTracing/arcane)**. For layout across services and how to contribute, see the root [`README.md`](https://github.com/ArcaneTracing/arcane/blob/main/README.md) and [`CONTRIBUTING.md`](https://github.com/ArcaneTracing/arcane/blob/main/CONTRIBUTING.md).

Backend API for Arcane — built with [NestJS](https://nestjs.com/), TypeScript, PostgreSQL, and optional integrations with RabbitMQ/Kafka, Redis, and OpenTelemetry.
...
From the **monorepo root**, use `backend/localsetup`. This starts PostgreSQL with `arcanedb` (user/password from `.env`).
...
The `localsetup/` directory (under this package) provides:
...
For a **full stack** (frontend, backend, worker, trace backends), use **[arcane-deployment](https://github.com/ArcaneTracing/arcane-deployment)** (Docker Compose in a separate repository).
