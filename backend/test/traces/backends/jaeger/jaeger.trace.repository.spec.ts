...
describe("JaegerTraceRepository", () => {
  ...
  describe("search", () => {
    ...
    it("should transform Jaeger response to Tempo format and preserve all attributes", async () => {
      ...
      expect(mockDatasourceAuthService.buildAuthHeaders).toHaveBeenCalledWith(
        mockDatasource,
      );
    ...
    it("should merge auth headers from DatasourceAuthService into request", async () => {
      mockDatasourceAuthService.buildAuthHeaders.mockReturnValue({
      ...
      await repository.search(mockDatasource, searchParams);
