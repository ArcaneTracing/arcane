...
describe("JaegerConversationRepository", () => {
  ...
  it("should extract conversations from span attributes when tags are empty", async () => {
    const repo = new JaegerConversationRepository(
      mockJaegerTraceRepository as any,
    );
    mockJaegerTraceRepository.search.mockResolvedValue({
    ...
    expect(result[0].conversationId).toBe("conv-from-span");
    expect(result[0].name).toBe("LangGraph");
    expect(result[0].traceIds).toContain("trace-1");
  });

  it("should extract conversations from span attributes with object value formats", async () => {
    const repo = new JaegerConversationRepository(
      mockJaegerTraceRepository as any,
    );
    mockJaegerTraceRepository.search.mockResolvedValue({
            ...
            spans: [
              {
                attributes: [
                  {
                    key: "operator.metadata.chatId",
                    value: { stringValue: "uuid-from-span" },
                  },
                ],
              },
            ],
    ...
    expect(result).toHaveLength(1);
    expect(result[0].conversationId).toBe("uuid-from-span");
  });
  ...
  describe("getFullConversation", () => {
    ...
    it("should match value from span attributes when not in tags", async () => {
      const repo = new JaegerConversationRepository(
        mockJaegerTraceRepository as any,
      );
      mockJaegerTraceRepository.search.mockResolvedValue({
